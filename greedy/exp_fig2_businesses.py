"""
Experiment: Fig2 equivalent for Businesses (F3 and F5).
Runs the Greedy baseline; EntrRed and Random results are loaded from prior experiments.
Results written to greedy/fig2_businesses/Discrete/ and greedy/fig2_businesses/Range/

Note: When use_MGTs=True the actual document text is unused; only len(documents)=n matters.
"""
import sys, os
...
CONFIGS = [
    {"name": "F3", "relevance_definition": "Type_of_food",            "diversity_definition": "Open_hours"},
    {"name": "F5", "relevance_definition": "Location_Around_New_York", "diversity_definition": "Cost"},
]

BASE_OUT = os.path.join(os.path.dirname(__file__), "fig2_businesses")
...
for setting, is_multiple_llms, orig_dir in [("Discrete", False, ORIG_DISCRETE), ("Range", True, ORIG_RANGE)]:
    ...
    for cfg in CONFIGS:
        ...
        out_file = os.path.join(out_dir, f"Results_Businesses_REL_{rel_def}_DIV_{div_def}.csv")
        orig_file = os.path.join(orig_dir, f"Results_Businesses_REL_{rel_def}_DIV_{div_def}.csv")

        print(f"\n=== Businesses {cfg['name']} | {setting} ===")
